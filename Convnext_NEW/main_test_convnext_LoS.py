...
mat = h5py.File('./datasets/angle/40m_NLoS.mat', 'r')  # 读取文件，得到字典
...
G.load_state_dict(torch.load('./model_pt/convnext_4_1.pt'))
...
for epoch in range(EPOCH):
    ...
    for i, x in enumerate(train_loader, 0):
        ...
        if Step % 20 == 0:
            print("[epoch %d][%d/%d]   RMSE_r: %.8f   RMSE_theta: %.8f   RMSE_d: %.8f   Time: %.4f " % (epoch + 1, i + 1, len(train_loader),  RMSE_r, RMSE_theta,RMSE_2D, (T2 - T1)))
