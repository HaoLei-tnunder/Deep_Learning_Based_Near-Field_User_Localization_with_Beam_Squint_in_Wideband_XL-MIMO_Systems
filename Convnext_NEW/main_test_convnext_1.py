...
from functional import pre_reshape_11, compute_MSE_r_test, compute_MSE_theta_test, compute_MSE_2D, fft_shrink, add_noise_improve, real_imag_stack, tensor_reshape, Beam_Squint_trajectory, CBS_theta
...
# mat = h5py.File('./datasets/test2/test_1000_5m_50m_y2_1_30dB.mat', 'r')  # 读取文件，得到字典
# x_train = mat['x1']  # 获取H_ori数据
# x_train = np.transpose(x_train)
# print(np.shape(x_train))
#
# mat4 = h5py.File('./datasets/test2/test_1000_5m_50m_y2_5_30dB.mat', 'r')  # 读取文件，得到字典
# x_train4 = mat4['x5']  # 获取H_ori数据
# x_train4 = np.transpose(x_train4)
# print(np.shape(x_train4))
#
# mat5 = h5py.File('./datasets/test2/test_1000_5m_50m_y2_6_30dB.mat', 'r')  # 读取文件，得到字典
# x_train5 = mat5['x6']  # 获取H_ori数据
# x_train5 = np.transpose(x_train5)
# print(np.shape(x_train5))
#
# mat6 = h5py.File('./datasets/test2/test_1000_5m_50m_y2_label_30dB.mat', 'r')  # 读取文件，得到字典
# x_train6 = mat6['label']  # 获取H_ori数据
# x_train6 = np.transpose(x_train6)
# print(np.shape(x_train6))
#
# H_get = pre_reshape_11(x_train, x_train4, x_train5, x_train6, img_height, img_width*2)
# print(np.shape(H_get))  # 4000 64 32
# train_loader = data.DataLoader(dataset=H_get, batch_size=BATCH_SIZE, shuffle=True)

# G_net = ConvNeXt(in_chans=3, depths=[3, 3, 27, 3], dims=[128,256,512,1024], num_classes=2)
G_net = ConvNeXt(in_chans=8, depths=[3, 3, 27, 3], dims=[96, 192, 384, 768], num_classes=2)
device_ids = [0]
# print(G_net)
...
G.load_state_dict(torch.load('./model_pt/convnext_3_2.pt'))
...
for epoch in range(EPOCH):
    ...
    for i, x in enumerate(train_loader, 0):
        ...
        real_img = real_img + x[:, 3, :, :]
        # print(real_img[:, 0, :2])
        # print(np.shape(x[:, 0, :, :]))
        real_img = Variable(real_img.cuda())  # H

        fake_img = G(x[:,:3,:,:])  # 随机噪声输入到生成器中，得到一副假的图片   4*2
